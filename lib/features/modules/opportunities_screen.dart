...
class _OpportunitiesScreenState extends State<OpportunitiesScreen> {
  ...
  Future<void> _updateLeadType(
    ...
    final bool ok = await widget.apiService.updateClient(
      ...
      company:
          (client['company'] ?? '').toString().isEmpty
              ? null
              : (client['company'] ?? '').toString(),
      email:
          (client['email'] ?? '').toString().isEmpty
              ? null
              : (client['email'] ?? '').toString(),
      phone:
          (client['phone'] ?? '').toString().isEmpty
              ? null
              : (client['phone'] ?? '').toString(),
      notes:
          (client['notes'] ?? '').toString().isEmpty
              ? null
              : (client['notes'] ?? '').toString(),
      leadTypeId: leadTypeId,
      leadSource:
          (client['lead_source'] ?? '').toString().isEmpty
              ? null
              : (client['lead_source'] ?? '').toString(),
      leadChannel:
          (client['lead_channel'] ?? '').toString().isEmpty
              ? null
              : (client['lead_channel'] ?? '').toString(),
      leadMessage:
          (client['lead_message'] ?? '').toString().isEmpty
              ? null
  ...
  Widget build(BuildContext context) {
    final List<Map<String, dynamic>> filtered =
        clients.where((client) {
          ...
          return (client['name'] ?? '').toString().toLowerCase().contains(
                keyword,
              ) ||
              (client['company'] ?? '').toString().toLowerCase().contains(
                keyword,
              ) ||
        ...
        child: ListView(
          ...
          children: <Widget>[
            StitchFilterCard(
              title: 'Bộ lọc cơ hội',
              subtitle:
                  'Theo dõi phễu khách hàng tiềm năng theo từ khóa và trạng thái lead.',
              ...
              child: Column(
                ...
                children: <Widget>[
                  StitchFilterField(
                    label: 'Từ khóa',
                    hint:
                        'Nhập tên khách hàng, công ty hoặc ghi chú để lọc nhanh phễu.',
                    child: TextField(
